"""Convert satellite-imagery masks to the common YOLO segmentation format."""
...
from mappings import SATELLITE_MAP


def default_directories(dataset_root: Path) -> Tuple[Path, Path]:
    candidates = (
        (dataset_root / "satellite" / "data", dataset_root / "satellite" / "labels"),
        (dataset_root / "satellite" / "images", dataset_root / "satellite" / "labels"),
        (dataset_root / "data", dataset_root / "labels"),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--clean",
        action="store_true",
        help="replace an existing satellite output directory",
    )
...
def main() -> None:
    ...
    output_dir = args.out_root / "satellite"
